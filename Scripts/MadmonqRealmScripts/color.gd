...
# HEALTH + DEATH CODE
	...
	if (red + green + blue + purple + yellow >= 5) and $".."/".."/Forsen.health > 0:
		$"..".state = $"..".DEATH
# CHANGE STATS CODE
	$".."/".."/Forsen.pdamage = 1 -(red*0.1) +(purple*0.2)
...
func _on_Timer_timeout():
	...
	if $Labels/yellowlabel.visible == true:
		...
		yellowlabel = false
		if yellow == 1:
			$".."/".."/Forsen/Sprite2.visible = true
		if yellow == 2:
			$".."/".."/Forsen/Sprite3.visible = true
		if yellow == 3:
			$".."/".."/Forsen/Sprite4.visible = true
		if yellow == 4:
			$".."/".."/Forsen/Sprite5.visible = true
